...
#define ADC_RES                 4096 //!< Maximum digital value for 10-bit ADC conversion.
#define ADC_PRE_SCALING_COMPENSATION   6    //!< The ADC is configured to use VDD with 1/3 prescaling as input. And hence the result of conversion is to be multiplied by 3 to get the actual value of the battery voltage.
#define VOLATAGE_DIVIDER 7.47

#define ADC_RESULT_IN_MILLI_VOLTS(ADC_VALUE) \
    ((((ADC_VALUE) *ADC_REF_VOLTAGE_IN_MILLIVOLTS) / ADC_RES) * ADC_PRE_SCALING_COMPENSATION * VOLATAGE_DIVIDER)
...
static void saadc_event_handler(nrf_drv_saadc_evt_t const *p_evt) {
  if (p_evt->type == NRF_DRV_SAADC_EVT_DONE) {
    ...
    NRF_LOG_INFO("ADC: %d, mV: %d", adc_result, m_batt_lvl_in_milli_volts);
  }
}
...
void battery_read_thread(void *arg) {
...
void battery_voltage_init(void) {
  ...
  xTaskCreate(battery_read_thread, "adc_thread", 1024, NULL, 1, NULL);
}
